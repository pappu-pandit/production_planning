...
from .models import  item_master,production_planning,opening_balance,production_damage,production_completed,production_order,item_master_copy,item_master_main,closing_balance,material_requisition,mrp_linewise_store,mrp_linewise_sponge,mrp_linewise_bms,mrp_store_opening,mrp_store_closing,mrp_sponge_opening,mrp_sponge_closing,mrp_bms_closing,mrp_bms_opening,\
production_done,gantt_line1,gantt_item_master,gantt_lines
...
def product_wise_mrp(request):
    today = fdate.today()
    product_wise_dict=[]

    master_copy=item_master_copy.objects.filter(date=today)

    for master_item in master_copy:
        coupli=0
        ladhi=0
        b=[]
        ...
        #print(itemcode)
        plan_data=production_planning.objects.filter(date=today,itemcode=itemcode).aggregate(Sum('quantity'))
        given_store=mrp_linewise_store.objects.filter(date=today,itemname=itemname).aggregate(Sum('quantity'))
        given_sponge = mrp_linewise_sponge.objects.filter(date=today, itemname=itemname).aggregate(Sum('quantity'))


        given_stored = given_store['quantity__sum']
        given_sponged = given_sponge['quantity__sum']



        if given_stored == None:
            given_stored = 0

        if given_sponged == None:
            given_sponged = 0

        print("sponge:::",given_sponged)

        if plan_data['quantity__sum'] != None:
            total_ladi = math.ceil(plan_data['quantity__sum'] / master_item.pics_per_ladi)
            # print(total_ladi)
            total_quantity = math.ceil((total_ladi * master_item.pics_per_ladi) / int(master_item.pics_per_coupli))
            coupli = coupli + total_quantity
            ladhi = ladhi + total_ladi

            #print(coupli,ladhi)
            dit = {"itemname": master_item.itemname, "coupli":master_item.coupli_or_board_size,"store": coupli,"given_store":given_stored,"sponge_type":master_item.sponge_type,"sponge_shape":master_item.sponge_shape ,"sponge": ladhi,"given_sponge":given_sponged}
            mrp_data=material_requisition.objects.filter(mr_parameter="layer_finish_decor")
            for ldf_item in mrp_data:
                given_bms = mrp_linewise_bms.objects.filter(date=today, itemname=itemname, lyf=ldf_item.mr_entity_name).aggregate(Sum('quantity'))
                given_bmsd = given_bms['quantity__sum']
                ...
                for i in range(1, 7):
                    if i == 1:
                        if ldf_item.mr_entity_name == master_item.l1:
                            s = s + master_item.l1_qty
                    if i == 2:
                        if ldf_item.mr_entity_name == master_item.l2:
                            s = s + master_item.l2_qty
                    if i == 3:
                        if ldf_item.mr_entity_name == master_item.l3:
                            s = s + master_item.l3_qty
                    if i == 4:
                        if ldf_item.mr_entity_name == master_item.l4:
                            s = s + master_item.l4_qty
                    if i == 5:
                        if ldf_item.mr_entity_name == master_item.l5:
                            s = s + master_item.l5_qty
                    if i == 6:
                        if ldf_item.mr_entity_name == master_item.l6:
                            s = s + master_item.l6_qty

                for i in range(1, 7):
                    if i == 1:
                        if ldf_item.mr_entity_name == master_item.f1:
                            s = s + master_item.f1_qty
                    if i == 2:
                        if ldf_item.mr_entity_name == master_item.f2:
                            s = s + master_item.f2_qty
                    if i == 3:
                        if ldf_item.mr_entity_name == master_item.f3:
                            s = s + master_item.f3_qty
                    if i == 4:
                        if ldf_item.mr_entity_name == master_item.f4:
                            s = s + master_item.f4_qty
                    if i == 5:
                        if ldf_item.mr_entity_name == master_item.f5:
                            s = s + master_item.f5_qty
                    if i == 6:
                        if ldf_item.mr_entity_name == master_item.f6:
                            s = s + master_item.f6_qty

                for i in range(1, 7):
                    if i == 1:
                        if ldf_item.mr_entity_name == master_item.d1:
                            s = s + master_item.d1_qty
                    if i == 2:
                        if ldf_item.mr_entity_name == master_item.d2:
                            s = s + master_item.d2_qty
                    if i == 3:
                        if ldf_item.mr_entity_name == master_item.d3:
                            s = s + master_item.d3_qty
                    if i == 4:
                        if ldf_item.mr_entity_name == master_item.d4:
                            s = s + master_item.d4_qty
                    if i == 5:
                        if ldf_item.mr_entity_name == master_item.d5:
                            s = s + master_item.d5_qty
                    if i == 6:
                        if ldf_item.mr_entity_name == master_item.d6:
                            s = s + master_item.d6_qty
                if s !=0:

                    #dit.update({ldf_item.mr_entity_name:s*ladhi})
                    a=(ldf_item.mr_entity_name,s*ladhi,given_bmsd)
                    b.append(a)
                    given_bmsd=0
                    #print(b)
            ...
            #print(dit)
            ditcopy = dit.copy()
            product_wise_dict.append(ditcopy)
    print(product_wise_dict)
    return render(request,'product_wise_mrp.html',{"data":product_wise_dict})
...
def material_requisition_sponge(request):
        ...
        for sp_shape_item in sp_shape_list:
            ...
            for i in tt:
                ...
                if i.station_no=='Line1':
                    ...
                    print('total_ladi micro',total_ladi , micro)
                    print(t2)
                elif i.station_no == "Line3":
    ...
    def spongeadd(dit):
        ...
        data2=[i for i in data1 if i['stn'] !='']
        #print(data2)
        #print(data1)
        ...
        for data in data2:
            ...
            line = data['stn']
            #print("line=",line)
            lineno = line[len(line) - 1]
            #print("STN=",lineno)

            stn = 'station' + str(lineno)
            #print(stn)
            ...
            if dit['sponge_type']=='dark_veg' and dit['sponge_shape']=='ladhi':
                ...
                dit[stn] = dit[stn] + dark_veg_ladhi
                dark_veg_ladhi = 0
                #print(dit)
...
def material_requisition_bms(request):
    ...
    if request.is_ajax() and request.method == "POST" or request.method == "POST":
        ...
        mrp_linewise_bms.objects.create(lyf=lyf,itemname=itemname,station_no=station, quantity=qty,date=today)
    ...
    for ldf_item in lfd_data:
        ...
        line = 0
        line_list = set()
        for master_item in item_master_data:
            micro = 0
            b = ''
            plan=production_planning.objects.filter(date=today,itemcode=master_item.itemcode).aggregate(Sum('quantity'))
            sett = {y for y in line_list if y == master_item.station_no}
            if sett == set():
                pass
            else:
                a = iter(sett)
                ...
                if b:
                    print('b=', b)
            if b == master_item.station_no:
                pass
            else:
                if plan['quantity__sum']:
                    mrp = mrp_linewise_bms.objects.filter(date=today, station_no=master_item.station_no,lyf=ldf_item.mr_entity_name).aggregate(Sum('quantity'))
                    micro = mrp['quantity__sum']

                    if micro == None:
                        micro = 0
                    line_list.add(master_item.station_no)

            qty=plan['quantity__sum']
            if qty==None:
                qty=0
            line = line + micro
            total_ladi = math.ceil(qty / master_item.pics_per_ladi)
            ...
            if master_item.station_no == 'Line1':
                t1 = t1 + t - micro
            elif master_item.station_no == "Line2":
                t2 = t2 + t - micro
            elif master_item.station_no == "Line3":
                t3 = t3 + t - micro
            elif master_item.station_no == "Line4":
                t4 = t4 + t - micro
            elif master_item.station_no == "Line5":
                t5 = t5 + t - micro
            elif master_item.station_no == "Line6":
                t6 = t6 + t - micro
            elif master_item.station_no == "Line7":
                t7 = t7 + t - micro
            elif master_item.station_no == "Line8":
                t8 = t8 + t - micro
            elif master_item.station_no == "Line9":
                t9 = t9 + t - micro
            else:
                pass
            k=k+t
            #print("row:",k)
            #print("total row",t,master_item.itemcode,ldf_item.mr_entity_name)
        l1 = l1 + t1
        l2 = l2 + t2
        l3 = l3 + t3
        l4 = l4 + t4
    ...
    line_dict = {"total": "Total", "line1": l1, "line2": l2, "line3": l3, "line4": l4, "line5": l5, "line6": l6,
                 "line7": l7, "line8": l8,
                 "line9": l9}
    return render(request, 'bms.html', {'data': dict_list,'line_dict':line_dict,'lfd':lfd, 'date': today})
            ...
            if itemcode=='copy':
                ...
                for data in item_master_data:
                    ...
                    item_master_copy.objects.create(itemcode=data.itemcode, itemname=data.itemname,
                                               status=data.status, station_no=data.station_no,duration=data.duration, pics_per_ladi=data.pics_per_ladi,
                                               ...
                                               l7=data.l7, l7_qty=data.l7_qty,

                                               f1=data.f1, f1_qty=data.f1_qty, f2=data.f2,f2_qty=data.f2_qty, f3=data.f3,
                                               f3_qty=data.f3_qty,f4=data.f4, f4_qty=data.f4_qty,f5=data.f5, f5_qty=data.f5_qty,
                                               f6=data.f6,f6_qty=data.f6_qty,f7=data.f7,f7_qty=data.f7_qty,

                                               d1=data.d1, d1_qty=data.d1_qty,d2=data.d2, d2_qty=data.d2_qty,
                                               d3=data.d3, d3_qty=data.d3_qty, d4=data.d4,d4_qty=data.d4_qty,
                                               d5=data.d5, d5_qty=data.d5_qty,d6=data.d6, d6_qty=data.d6_qty,
                                               d7=data.d7, d7_qty=data.d7_qty,
    ...
    if request.user.is_authenticated:
        ...
        if request.method == 'POST':
            ...
            if itemcode and status and station and pics_per_ladi:
                ...
                item_master.objects.create(itemcode=itemcode[0].itemcode,itemname=itemcode[0].itemname,status=status,station_no=station,duration=duration,pics_per_ladi=pics_per_ladi,
                ...
                l1=l1,l1_qty=l1_qty,l2=l2,l2_qty=l2_qty,l3=l3,l3_qty=l3_qty,l4=l4,l4_qty=l4_qty,l5=l5,l5_qty=l5_qty,l6=l6,l6_qty=l6_qty,l7=l7,l7_qty=l7_qty,
                f1=f1, f1_qty=f1_qty,f2=f2, f2_qty=f2_qty,f3=f3, f3_qty=f3_qty,f4=f4, f4_qty=f4_qty,f5=f5, f5_qty=f5_qty,f6=f6, f6_qty=f6_qty,f7=f7, f7_qty=f7_qty,
                d1=d1, d1_qty=d1_qty, d2=d2, d2_qty=d2_qty,d3=d3, d3_qty=d3_qty, d4=d4, d4_qty=d4_qty, d5=d5, d5_qty=d5_qty, d6=d6, d6_qty=d6_qty,d7=d7, d7_qty=d7_qty,item_master_align_id=id)
    ...
    if request.user.is_authenticated:
        ...
        if todayy == today or todayy==tomorrow:
            ...
            for data in yes_data:
                print("deepak")
                item_master_copy.objects.create(itemcode=data.itemcode, itemname=data.itemname,status=data.status, station_no=data.station_no,duration=data.duration,pics_per_ladi=data.pics_per_ladi,
...
def gantt(request,line):
    today = fdate.today()
    tomorrow = fdate.today() + timedelta(days=1)
    yesterday = fdate.today() - timedelta(days=1)

    master=gantt_item_master.objects.last()
    ...
    if master:
        ...
        if master.date == today:
            ...
            gantt_item_master.objects.all().delete()

            for i in ['Line1', 'Line2', 'Line3', 'Line4', 'Line5', 'Line6', 'Line7', 'Line8', 'Line9']:
                master1 = item_master_copy.objects.filter(date=today, station_no=i)
                k = 0
                for j in master1:
                    k += 1
                    gantt_item_master.objects.create(itmcode=j.itemcode, itemname=j.itemname, station_no=j.station_no,temp_id=k, duration=j.duration, date=today)
    else:
        for i in ['Line1', 'Line2', 'Line3', 'Line4', 'Line5', 'Line6', 'Line7', 'Line8', 'Line9']:
            master1 = item_master_copy.objects.filter(date=today, station_no=i)
            k = 0
    ...
    count= gantt_item_master.objects.filter(station_no=line).count()
    svg_hgt=''
    if count:
        svg_hgt=85 * count
    ...
    data = datetime.now(IST) + timedelta(minutes=0)
    ...
    if request.is_ajax() and request.method=='GET':
        ...
        dit_list=[]
        #timee="09:20"
        s=gantt_item_master.objects.filter(station_no=line).order_by('temp_id')
        stime=gantt_lines.objects.filter(stn_line=line)
        sTime=stime[0].starttime

        timee=sTime.strftime("%H:%M")
        timee1 = sTime.strftime("%H")
        timee2 = sTime.strftime("%M")
        print(timee,timee1,timee2)


        lastt=production_done.objects.filter(station_no=line).last()
        #print(lastt.itemcode)

        if line=="Line1":
            fp=open("C:/django/production_planning_v1.2/ppf_project/gantt_file1.txt", "r")
        elif line=="Line2":
            fp = open("C:/django/production_planning_v1.2/ppf_project/gantt_file2.txt", "r")
        elif line == "Line3":
            fp = open("C:/django/production_planning_v1.2/ppf_project/gantt_file3.txt", "r")
        elif line == "Line4":
            fp = open("C:/django/production_planning_v1.2/ppf_project/gantt_file4.txt", "r")
        elif line == "Line5":
            fp = open("C:/django/production_planning_v1.2/ppf_project/gantt_file5.txt", "r")
        elif line == "Line6":
            fp = open("C:/django/production_planning_v1.2/ppf_project/gantt_file6.txt", "r")
        elif line == "Line7":
            fp = open("C:/django/production_planning_v1.2/ppf_project/gantt_file7.txt", "r")
        elif line == "Line8":
            fp = open("C:/django/production_planning_v1.2/ppf_project/gantt_file8.txt", "r")
        elif line == "Line9":
            fp = open("C:/django/production_planning_v1.2/ppf_project/gantt_file9.txt", "r")
        else:
            pass

        text = fp.read()
        print(len(text),len(lastt.itemcode))
        fp.close()



        if text==lastt.itemcode:


            sa=gantt_line1.objects.filter(itemcode=lastt.itemcode,date=today,stn_line=line).update(endtime=current_time,qty=lastt.qty,planned=planned)


        else:
            print("===========")
            ss=gantt_line1.objects.filter(date=today,stn_line=line).last()
            if ss:
                timee=ss.endtime
            gantt_line1.objects.create(itemcode=lastt.itemcode,itemname="KAJU KISMIS VEG PASTRY",starttime=timee,endtime=current_time,status='running',qty=0,planned=0,date=today,stn_line=line)
            if text:
                gantt_line1.objects.filter(itemcode=text,date=today,stn_line=line).update(status='Done')

            #fp = open("C:/django/production_planning_v1.2/ppf_project/gantt_file.txt", "w")
            if line == "Line1":
                fp = open("C:/django/production_planning_v1.2/ppf_project/gantt_file1.txt", "w")
            elif line == "Line2":
                fp = open("C:/django/production_planning_v1.2/ppf_project/gantt_file2.txt", "w")
            elif line == "Line3":
                fp = open("C:/django/production_planning_v1.2/ppf_project/gantt_file3.txt", "w")
            elif line == "Line4":
                fp = open("C:/django/production_planning_v1.2/ppf_project/gantt_file4.txt", "w")
            elif line == "Line5":
                fp = open("C:/django/production_planning_v1.2/ppf_project/gantt_file5.txt", "w")
            elif line == "Line6":
                fp = open("C:/django/production_planning_v1.2/ppf_project/gantt_file6.txt", "w")
            elif line == "Line7":
                fp = open("C:/django/production_planning_v1.2/ppf_project/gantt_file7.txt", "w")
            elif line == "Line8":
                fp = open("C:/django/production_planning_v1.2/ppf_project/gantt_file8.txt", "w")
            elif line == "Line9":
                fp = open("C:/django/production_planning_v1.2/ppf_project/gantt_file9.txt", "w")
            else:
                pass

            fp.seek(0)
            fp.write(lastt.itemcode)
            fp.close()
            timee=str(timee)
            timee=timee[:5]

        ct=gantt_line1.objects.filter(date=today, stn_line=line)
        m=0
        for i in ct:
            m += 1
            itemcode=i.itemcode
            print(today,line,m)
            data=gantt_item_master.objects.get(date=today,station_no=line,temp_id=m)

            data1= gantt_item_master.objects.get(itmcode=itemcode,date=today,station_no=line)
            print(data1)
            gantt_item_master.objects.filter(itmcode=itemcode,date=today,station_no=line).update(temp_id=m)
            gantt_item_master.objects.filter(date=today, station_no=line,id=data.id).update(temp_id=data1.temp_id)


        if current_time>=timee:
            hr = min = 0
            data1 = datetime(2021,7,1,int(timee1),int(timee2),0) + timedelta(hours=hr, minutes=min)
            status = '/200 ,ETF:23:12'
            itm1 = gantt_line1.objects.filter(stn_line=line,date=today).last()
            if itm1:
                line_status=itm1.status
                print(line_status)

            for i in s:
                caltime=i.duration
                data=round(int(caltime)*planned/60)
                min=data%60
                hr=data//60
                data2=data1 + timedelta(hours=hr, minutes=min)
                current_time1 = data2.strftime("%H:%M")

                check_data=round(int(caltime)*(planned - lastt.qty)/60)
                mint = check_data % 60
                hrr = check_data // 60
                etf = datetime.now(IST) + timedelta(hours=hrr, minutes=mint)
                etf_time = etf.strftime("%H:%M")

                print('ETF=',etf_time)

                data=i.itmcode
                itm = gantt_line1.objects.filter(itemcode=data.strip(),status="Done",date=today,stn_line=line)


                if itm:
                    print('true=',itm[0].itemcode)
                    if itm[0].itemcode:
                        print("code=",data.strip(),itm[0].itemcode)
                        time1=str(itm[0].starttime)
                        time1=time1[:5]

                        time2 = str(itm[0].endtime)
                        time2 = time2[:5]

                        pdata=str(planned - itm[0].qty)
                        if pdata.startswith('-'):
                            pdata=-int(pdata)
                        else:
                        ...
                        dit={'task':timee+'='+current_time1,'type':itm[0].itemname,'startTime':timee,'endTime':current_time1,'details': 'all three lines of it'}
                        dit1 = {'task': str(pdata)+',Done', 'type': itm[0].itemname, 'startTime': time1, 'endTime':time2,'details': 'all three lines of it'}
                        current_time1=time2
                        timee = current_time1
                        #print('time2=',timee)
                        ...
                        if current_time >= timee:
                            l=timee[:2]
                            k=timee[3:5]
                            data2 = datetime(2021, 7, 1, int(l), int(k), 0)
                else:
                    if line_status=='running':
                        dit = {'task': timee + '=' + current_time1, 'type': itm1.itemname, 'startTime': timee,'endTime': current_time1, 'details': 'all three lines of it'}
                        dit1 = {'task': str(itm1.qty) + '/'+ str(planned) + ',ETF:'+ etf_time, 'type':itm1.itemname, 'startTime': timee, 'endTime': current_time,'details': 'all three lines of it'}
                        line_status=''
                    else:
                        #current_time="10:00"
                        dit = {'task': timee + '=' + current_time1, 'type': i.itemname, 'startTime': timee,'endTime': current_time1, 'details': 'all three lines of it'}
                        dit1 = {'task': '', 'type': i.itemname, 'startTime': timee, 'endTime': current_time,'details': 'all three lines of it'}

                    timee = current_time1
                    if current_time > timee:
                        timee = current_time
                        data2 = datetime(2021, 7, 1, int(h), int(m), 0)
                ...
                dit_list.append(ditcopy1)
                dit_list.append(ditcopy2)


        data=json.dumps(dit_list)
        #print(data)
    ...
    return render(request,'gantt.html',{'Line':line,'svg_hgt':svg_hgt})
