...
from .models import  item_master,production_planning,opening_balance,production_damage,production_completed,production_order,item_master_copy,item_master_main,closing_balance,material_requisition,mrp_linewise_store,mrp_linewise_sponge,mrp_linewise_bms
...
def console(request):
    ...
    results =sync_to_async(production_planning.objects.all, thread_sensitive=True)
    print(results)



    return render(request,'embedd.html',{'form':fm})
def micro_mrp(request):
    return render(request,'micro_mrp.html')
...
def material_requisition_store(request):
    ...
    for list_coupli_board in coupli_list:
        ...
        if tt:
            ...
            #print("foor loop end")
            dit={'coupli_board_size':list_coupli_board.mr_entity_name,'quantity':t,"given":line,"station1":t1,"station2":t2,"station3":t3,"station4":t4,
                     "station5":t5,"station6":t6,"station7":t7,"station8":t8,"station9":t9}
            #print(dit)
            ditcopy=dit.copy()
            dict_list.append(ditcopy)
        else:
            dit = {'coupli_board_size': list_coupli_board.mr_entity_name, 'quantity': 0,"given":0,"station1":0,"station2":0,"station3":0,"station4":0,
                     "station5":0,"station6":0,"station7":0,"station8":0,"station9":0}
...
def material_requisition_sponge(request):
    ...
    if request.is_ajax() and request.method == "POST" or request.method=="POST":
        ...
        mrp_linewise_sponge.objects.create(sponge_type=sp_type,sponge_shape=sp_shape,quantity=qty,station_no=station,date=today)
    ...
    for sp_type_item in sp_type_list:
        ...
        for sp_shape_item in sp_shape_list:
            ...
            dit={"id":index,"sponge_type":sp_type_item.mr_entity_name,"sponge_shape":sp_shape_item.mr_entity_name,"quantity":t,"given":line,"station1":t1,"station2":t2,"station3":t3,"station4":t4,
                     "station5":t5,"station6":t6,"station7":t7,"station8":t8,"station9":t9,"stn":stn_no}
            ditcopy=dit.copy()
            dict_list.append(ditcopy)
            index=index+1
    ...
    #print(dict_list)
    print(l1)
    line_dict = {"total":"Total","line1": l1, "line2": l2, "line3": l3, "line4": l4, "line5": l5, "line6": l6, "line7": l7, "line8": l8,
                 "line9": l9}
    return render(request, 'sponge.html', {'data': dict_list,'line_dict':line_dict, 'date': today})
...
def material_requisition_bms(request):
    ...
    for ldf_item in lfd_data:
        ...
        l8 = l8 + t8
        l9 = l9 + t9
        dit = {'lfd_items': ldf_item.mr_entity_name,  'quantity': k,"given":line,"station1":t1,"station2":t2,"station3":t3,"station4":t4,
                     "station5":t5,"station6":t6,"station7":t7,"station8":t8,"station9":t9}
        ditcopy = dit.copy()
        dict_list.append(ditcopy)
